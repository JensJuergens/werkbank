...
This question can neither be easily answered, nor are we able to make a crystal ball. But we can try to sketch the production profile future with statistical means in order to get a sensible feeling for the uncertainty in renewables production profiles. Hence my first post is about the setup of a nice, small model, which is capable of forecasting daily production volumes of renewable power plants while being analytically tractable. We will enhance and modify the model in the upcoming posts, so it will be the primary model for answering all questions about energy production. All calculations are done with R and are implementations of methods carried out in the cited academic papers.
## Data Description
...
But as this blog should be quantitative, let us see, what the numbers say in the end.
## The model
